# Hi, I'm WolfZix 👋  
### Frontend Developer focused on React & modern UI

<p align="left">
  <a href="https://github.com/WolfZix">
    <img src="https://komarev.com/ghpvc/?username=WolfZix&label=Profile%20views&color=0e75b6&style=flat" alt="profile views"/>
  </a>
</p>

---

# 🚀 About Me

Frontend Developer focused on building modern, responsive and interactive web applications using React ecosystem.

I enjoy creating clean UI, smooth animations and real-world projects that improve both user experience and performance.  
Currently expanding my knowledge in React architecture and advanced frontend development.

- ⚛️ Mainly working with React + Vite
- 🎨 Focused on UI/UX & animations
- 📚 Currently learning deeper React hooks and work with REST API's
- 🔍 Looking for my first opportunity as a Frontend Developer

---

# 🛠️ Tech Stack

### Frontend
![React](https://img.shields.io/badge/React-111827?style=for-the-badge&logo=react)
![JavaScript](https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-111827?style=for-the-badge&logo=typescript)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-111827?style=for-the-badge&logo=tailwindcss)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-111827?style=for-the-badge&logo=framer)

### Tools
![Vite](https://img.shields.io/badge/Vite-111827?style=for-the-badge&logo=vite)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github)

---

# 📌 Featured Projects

## 🌐 Portfolio Website <a href="https://wolfzixportfolio.pl/">![Check it out!](https://img.shields.io/badge/Check%20it%20out%20%F0%9F%91%89-%23BD0000?style=for-the-badge&logoColor=red&labelColor=red)</a>
Modern portfolio built with React and Framer Motion focused on smooth animations, responsive layout and clean UI.

## 📊 Dashboard App *(In Progress)* <a href="https://wolfzix.github.io/dashboard/"><img src="https://img.shields.io/badge/Check%20it%20out!" /></a>
Real-world dashboard project using React Router, React Hooks and reusable components architecture using Framer motion for animations.

## ✅ ToDo App <a href="https://wolfzix.github.io/todo-app/"><img src="https://img.shields.io/badge/Check%20it%20out!" /></a>
Task management application with dynamic state handling.

## ⏰ Digital Clock App <a href="https://wolfzix.github.io/digital-clock-app/"><img src="https://img.shields.io/badge/Check%20it%20out!" /></a>
Interactive digital clock application focused on time logic and UI presentation.

## 🎮 Tic Tac Toe <a href="https://wolfzix-tictactoe.netlify.app/"><img src="https://img.shields.io/badge/Check%20it%20out!" /></a>
Simple browser game project built with only HTML CSS and Javascript with menu and different background options.

## 🏓 Ping Pong <a href="https://wolfzix-pingpong.netlify.app/"><img src="https://img.shields.io/badge/Check%20it%20out!" /></a>
Simple browser game project focused on JavaScript logic and interactions with options menu, pvp or pve mode and different background options.

---

# 📖 Currently Learning

- React Hooks
- API integration
- Advanced animations
- UX / UI optimization
- Performance optimization

---

# 📈 GitHub Stats

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=WolfZix&show_icons=true&theme=tokyonight"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=WolfZix&layout=compact&theme=tokyonight"/>
</p>

---

# 🌍 Contact

### ➡️[Portfolio](https://wolfzixportfolio.pl/)

### ➡️ [LinkedIn](https://www.linkedin.com/in/patryk-domaszek-508126370/)

### Discord ➡️ WolfeZix

### ➡️ [Facebook](https://www.facebook.com/profile.php?id=100031876898312)

### Email ➡️ patryk.domaszek41@gmail.com

---

# ⚡ Fun Fact

I enjoy building interfaces that feel smooth, modern and satisfying to use.
